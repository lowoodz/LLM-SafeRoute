#!/usr/bin/env bash
# Bootstrap Windows x86_64 VM on macOS (Apple Silicon) for SafeRoute testing.
...
VM_DIR="${SMR_VM_DIR:-${HOME}/VMs/SafeRoute-Win11-x64}"
ISO_PATH="${VM_DIR}/isos/Win11_24H2_English_x64.iso"
VM_NAME="${SMR_VM_NAME:-SafeRoute-Win11-x64}"
...
SSH (after Windows is up with OpenSSH):
  Set in config/test.env (copy from config/test.env.example):
    ...
    SMR_WINDOWS_USER=your-windows-ssh-user
...
Environment:
  SMR_VM_DIR       VM files directory (default: ~/VMs/SafeRoute-Win11-x64)

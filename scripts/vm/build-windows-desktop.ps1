# Build SecureModelRoute Tauri desktop app on Windows (MSVC).
# CLI zip from macOS may use GNU (x86_64-pc-windows-gnu); desktop uses native MSVC on the build host.
$ErrorActionPreference = "Continue"
$LogPath = "C:\Users\Public\smr-desktop-build.log"

$TargetFile = "C:\Users\Public\smr-gui-target.txt"
...
# Drop cached x64 Node on ARM hosts (utmctl may run emulated x64 PowerShell).
if ((Get-OsArch) -eq "ARM64" -and (Test-Path "C:\Users\Public\node\node.exe")) {
    Remove-Item "C:\Users\Public\node" -Recurse -Force -ErrorAction SilentlyContinue
}

$SrcZip = "C:\Users\Public\smr-build-src.zip"
$BuildRoot = "C:\Users\Public\smr-build"
$OutDir = "C:\Users\Public\smr-desktop-out"
...
$TargetCache = "C:\Users\Public\smr-build-target-cache"
$OldTarget = "C:\Users\Public\smr-build\target"
...
if ($rustHost -eq "ARM64" -and (Test-Path "C:\Users\Public\node")) {
    Remove-Item "C:\Users\Public\node" -Recurse -Force -ErrorAction SilentlyContinue
}
...
if (-not (Test-Path (Join-Path $FrontendDist "index.html"))) {
    ...
    @'
<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>SecureModelRoute</title></head>
<body><p>Loading SecureModelRoute...</p>
...
Log "tauri build --target $TargetTriple --no-bundle (portable exe; skip NSIS/WiX download) ..."
npx tauri build --target $TargetTriple --no-bundle 2>&1 | ForEach-Object { Log $_ }
...
if (-not $BuiltExe) {
    $BuiltExe = Get-ChildItem $Release -Filter "SecureModelRoute.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
}
if (-not $buildOk -and $BuiltExe) {
    Log "WARNING: bundler failed but exe exists; continuing with portable app"
    $buildOk = $true
}
...
$AppExe = $BuiltExe
if (-not $AppExe) {
    $AppExe = Get-ChildItem $Release -Filter "SecureModelRoute.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
}
if (-not $AppExe) {
    $AppExe = Get-ChildItem $Release -Filter "smr-gui.exe" -ErrorAction SilentlyContinue | Select-Object -First 1
}
if (-not $AppExe) {
...
Copy-Item $AppExe.FullName (Join-Path $OutDir "SecureModelRoute.exe") -Force
